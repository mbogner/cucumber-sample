...
class AnotherFeature @Autowired constructor(
    ...
    fun the_client_issues_GET_version() {
        httpSession.executeForString(
            url = "http://localhost:$port/another",
    ...
    @Then("^the client also receives status code of (\\d+)$")
    ...
    fun the_client_receives_server_version_body(number: String) {
        assertThat(httpSession.getLatestResponse().body as String).isEqualTo(number)
    }
